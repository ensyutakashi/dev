...
# 1. 対象フォルダ
SOURCE_DIR = r"C:\Users\ensyu\Documents\Speculation\TDnet\TDnet適時情報開示サービス"
...
# 2. Obsidianの出力先フォルダ
OBSIDIAN_DIR = r"C:\Users\ensyu\Documents\Obsidian保管庫\ensyu_star_capital\python\TDnet適時情報開示サービス"
...
def run():
    ...
    if os.path.exists(OBSIDIAN_DIR):
        ...
        try:
            ...
            print(f"Obsidian出力先を完全にクリーンアップしました: {OBSIDIAN_DIR}")
        except Exception as e:
    ...
    for root, dirs, files in os.walk(SOURCE_DIR):
        ...
        for file in files:
            if file.endswith(".py") and file != "index_scripts.py":
                ...
                if folder_rel_path == '.':
                    obsidian_subdir = OBSIDIAN_DIR
                else:
                    obsidian_subdir = os.path.join(OBSIDIAN_DIR, folder_rel_path)
    ...
    obsidian_files = len([f for f in os.listdir(OBSIDIAN_DIR) if f.endswith('.md')]) if os.path.exists(OBSIDIAN_DIR) else 0
